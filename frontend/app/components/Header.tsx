  ...
  const mobileMenuPortal = (
    <div
      style={{
        ...
        background: 'linear-gradient(135deg, #111518 0%, #1a1d21 50%, #111518 100%)',
      }}
    >
      {/* Header row */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px' }}>
        <a href="/" onClick={closeMenu}>
          <img src="/images/slza_logo_biele.svg" alt="SLZA Print" style={{ height: '48px', width: 'auto' }} />
        </a>
        <button
          onClick={closeMenu}
          style={{
      ...
      <div style={{ padding: '0 32px 40px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {user ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'rgba(255,255,255,0.6)', fontSize: '14px', marginBottom: '4px' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#0087E3', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px', fontWeight: 700 }}>
                {user.firstName?.charAt(0)?.toUpperCase() || 'U'}
              </div>
              <span>{user.firstName} {user.lastName}</span>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <a href="/ucet" onClick={closeMenu} style={{ flex: 1, textAlign: 'center', padding: '12px', borderRadius: '12px', background: 'rgba(255,255,255,0.1)', color: 'white', textDecoration: 'none', fontSize: '14px', fontWeight: 500 }}>Môj účet</a>
              {isAdmin && (
                <a href="/admin" onClick={closeMenu} style={{ flex: 1, textAlign: 'center', padding: '12px', borderRadius: '12px', background: 'rgba(0,135,227,0.2)', color: '#0087E3', textDecoration: 'none', fontSize: '14px', fontWeight: 500 }}>Admin</a>
              )}
            </div>
            <button onClick={handleLogout} style={{ width: '100%', padding: '12px', borderRadius: '12px', background: 'rgba(239,68,68,0.1)', color: '#f87171', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 500 }}>
              Odhlásiť sa
            </button>
          </>
        ) : (
          <a href="/login" onClick={closeMenu} style={{ display: 'block', textAlign: 'center', padding: '14px', borderRadius: '12px', background: '#0087E3', color: 'white', textDecoration: 'none', fontSize: '14px', fontWeight: 600 }}>
            Prihlásiť sa
          </a>
        )}
      </div>
        ...
        <div className="max-w-[1320px] mx-auto px-5 flex justify-between items-center">
          ...
          <div className="lg:hidden flex items-center gap-3">
            ...
            <button
              ...
              className="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all"
              aria-label="Menu"
            >
          ...
          {/* Desktop icons */}
          <div className="hidden lg:flex gap-5 items-center flex-1 justify-end relative">
            <button
            ...
            {searchOpen && (
              ...
              >
                ...
                />
                <button type="submit" className="text-sm font-semibold text-[#0087E3] hover:text-[#006bb3]">
                  Hľadať
                </button>
              </form>
            )}
